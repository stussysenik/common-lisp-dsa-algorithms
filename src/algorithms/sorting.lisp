...
(defun heap-sort (vec &key (test #'<) (key #'identity))
  ...
  (let* ((arr (copy-seq (coerce vec 'vector)))
    ...
    (labels ((sift-down (start end)
               (let ((root start))
                 (loop
                   (let* ((left-child (1+ (* 2 root)))
                          (right-child (+ 2 (* 2 root)))
                          (swap root))
                     (when (and (<= left-child end)
                                (funcall heap-test (aref arr left-child) (aref arr swap)))
                       (setf swap left-child))
                     (when (and (<= right-child end)
                                (funcall heap-test (aref arr right-child) (aref arr swap)))
                       (setf swap right-child))
                     (if (= swap root)
                         (return)
